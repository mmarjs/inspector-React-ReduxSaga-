...
class ChecksListScreen extends React.PureComponent<Props, State> {
    ...
    renderRow = (check: Check) => {
        ...
        return (
            <div className="card-wrapper" onClick={() => this.goToProduct(check)} key={check.check_id}>
                <Paper className={check.progress == '100' ? "card check-list complete" : "card check-list"}>
                    <div className="df">
                        {check.product_number ? <h2>№{check.product_number}</h2> : <h2>Создать зав. номер</h2>}
                        <div className="progress">{check.progress}%</div>
                    </div>
                    <div className="bold">{check.stage_title}</div>
                    <div className="card-section">
                        <span className="product-title">{contractCode} прил. {contractAnnex}</span>
                        <span className="product-title">{check.product_title} (позиция {check.product_position})</span>
                        <div className="info-row">
                            ...
                            <div className="item-info">
                                ...
                                <span className="value">{cargoDate}</span>
                            </div>
                            <div className="item-info">
